...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    /**
     * 创建一个新的任务队列，子类对象可以通过重写当前方法，来使用新的阻塞队列（如NioEventLoop）
     * <p>
    ...
    protected final boolean runAllTasksFrom(Queue<Runnable> taskQueue) {
        ...
        // 每次从获取任务队列的第一个任务，然后执行，直至任务队列为空
        for (; ; ) {
            safeExecute(task);
            task = pollTaskFrom(taskQueue);
            if (task == null) {
                // 这里返回true的原因是只要执行到这一步，已经保证有一个任务执行完成了
                return true;
            }
        }
    }
    ...
    protected boolean runAllTasks(long timeoutNanos) {
        ...
        // 计算当前任务截止时间：currentTime + timeOut
        ...
        for (; ; ) {
            ...
            // 由于每次获取nanoTime代价比较昂贵，因此没64个任务执行一次超时检查
            // 也就是第64次任务会将执行完成时间和deadline进行比较
            if ((runTasks & 0x3F) == 0) {
                lastExecutionTime = getCurrentTimeNanos();
    ...
    protected void wakeup(boolean inEventLoop) {
        // 如果当前方法在EventLoop线程中使用，则表示EventLoop线程正在工作，则无需唤醒
        if (!inEventLoop) {
    ...
    /**
     * 实际上并不是立即执行任务，而是将任务添加到任务队列中去。
     * 只有当前的线程是添加任务后不能自动唤醒且immediate属性为true时
     * 才会立即唤醒线程，执行任务。
     * <p>
     * 本质上来说都是通过线程调度来执行任务的
     *
    ...
    /**
     * 通过在当前线程中调用SingleThreadEventExecutor::run方法，来执行任务队列中的任务
     */
    private void doStartThread() {
        ...
        executor.execute(() -> {
            ...
            // 当前线程已经被标记打断，则执行线程的interrupt()方法
            if (interrupted) {
                thread.interrupt();
            }
            ...
            try {
                // 使用SingleThreadEventExecutor对象的run方法，来执行任务队列中的方法
